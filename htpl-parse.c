...
int parse_htpl_catch(stack, untag)
    ...
    makepersist(stack);
    if (numtokens > 1) RETURN(croak("CATCH called with %d arguments, maximum needed is 1", numtokens))
...
int parse_htpl_project(stack, untag)
    ...
    if (numtokens > 3) RETURN(croak("PROJECT called with %d arguments, maximum needed is 3", numtokens))
...
int parse_htpl_try___fwd(stack, untag)
    ...
    printcode("{\n");
    printcode("my %__htpl_handler, $__htpl_default_handler; \n");
    printcode("my $__htpl_try_sub = sub {\n");
    nesting = 0;
    RETURN(1)
}
